...
export const manageWorkforce = async (payload: any) => {
  try {
    ...
    return {
      success: true,
      message: 'manage workforce completed successfully.',
      data: payload
    };
  } catch (error: any) {
    console.error('[Action] Error in manageWorkforce:', error);
    return {
      success: false,
