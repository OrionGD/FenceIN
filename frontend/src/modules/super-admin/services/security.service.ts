...
export class SecurityService {
  ...
  static async getMany(filters?: Record<string, any>) {
    try {
      const response = await api.get('/api/v1/security', { params: filters });
  ...
  static async getById(id: string) {
    try {
      const response = await api.get(`/api/v1/security/${id}`);
      return response.data;
    } catch (error) {
  ...
  static async create(payload: any) {
    try {
      const response = await api.post('/api/v1/security', payload);
      return response.data;
    } catch (error) {
  ...
  static async update(id: string, payload: any) {
    try {
      const response = await api.patch(`/api/v1/security/${id}`, payload);
      return response.data;
    } catch (error) {
  ...
  static async delete(id: string) {
    try {
      const response = await api.delete(`/api/v1/security/${id}`);
      return response.data;
    } catch (error) {
