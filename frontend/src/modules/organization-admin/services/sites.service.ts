...
export class SitesService {
  ...
  static async getById(id: string) {
    try {
      ...
      console.error(`Error fetching sites ${id}:`, error);
      throw error;
    }
  }
  ...
  static async create(payload: any) {
    try {
      const response = await api.post('/api/v1/sites', payload);
      return response.data;
    } catch (error) {
  ...
  static async update(id: string, payload: any) {
    try {
      const response = await api.patch(`/api/v1/sites/${id}`, payload);
      return response.data;
    } catch (error) {
      console.error(`Error updating sites ${id}:`, error);
      throw error;
    }
  }
  ...
  static async delete(id: string) {
    try {
      ...
      console.error(`Error deleting sites ${id}:`, error);
      throw error;
    }
  }
}
